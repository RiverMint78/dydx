...
simplifyAdd le re = buildAdd merged
  where
    sorted = sortBy (comparing baseAdd) (flattenAdd le ++ flattenAdd re)
    merged = filter (/= Const 0) (mergeAddTerms sorted)
...
simplifyMul le re
    | Const 0 `elem` allTerms = Const 0
    | otherwise = buildMul merged
  where
    allTerms = flattenMul le ++ flattenMul re
    sorted = sortBy (comparing baseMul) (flattenMul le ++ flattenMul re)
    merged = filter (/= Const 1) (mergeMulTerms sorted)
